...
from .utils import had_mul, HadamardTransform


def add_hooks(model, meta):
    layers = None
    if meta.model_type == ModelType.VLM:
        layers = model.language_model.layers
    elif meta.model_type == ModelType.LLM:
        layers = model.model.layers
    for name, module in model.named_modules():
        if isinstance(module, torch.nn.Linear):
            if "lm_head" not in name:
                setattr(
                    get_nested_attr(model, ".".join(name.split(".")[:-1])),
                    name.split(".")[-1],
                    QuantizeLinearNEW(module),
                )
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, torch.nn.Embedding):
            ...
            def hook_fn(module, inputs, outputs):
                ...
                outputs = (
                    outputs.to(dtype=torch.float64)
                    @ model.R1.weight.to(dtype=torch.float64)
                ).to(dtype=original_dtype)
                return outputs

            module.register_forward_hook(hook_fn)
        if isinstance(module, LlamaModel) or isinstance(module, Qwen2_5_VLTextModel):

            def hook_fn2(module, inputs, outputs):
                original_dtype = outputs["last_hidden_state"].dtype
                outputs["last_hidden_state"] = (
                    outputs["last_hidden_state"].to(dtype=torch.float64)
                    @ model.R1.weight.T.to(dtype=torch.float64)
                ).to(dtype=original_dtype)
                return outputs

            module.register_forward_hook(hook_fn2)
        if isinstance(module, QuantizeLinearNEW):
            if "down" in name:
                module.register_forward_hook(
                    lambda module, inputs, output: module.forward(
                        inputs[0], model.R1.weight, transpose=True
                    )
                )
            elif "v_proj" in name:
                module.register_forward_hook(
                    lambda module, inputs, output: module.forward(
                        inputs[0],
                        model.R1.weight,
                        layers[module.layer_idx].self_attn.R2.weight,
                    )
                )
            elif "o_proj" in name:
                module.register_forward_hook(
                ...
                module.register_forward_hook(
                    lambda module, inputs, output: module.forward(
                        inputs[0], model.R1.weight
                    )
                )


@torch.no_grad()
def rtn_fwrd(model, meta, dev, args):
    """
    From GPTQ repo
    """
    ...
    for i in tqdm(range(len(layers)), desc="Inserting weight quantizer"):
        layer = layers[i].to(dev)

        subset = find_qlayers(layer, layers=[torch.nn.Linear, QuantizeLinearNEW])
...
class QuantizeLinearNEW(torch.nn.Module):
...
class ActQuantizer(torch.nn.Module):
    ...
    def configure(
        ...
        assert self.clip_ratio <= 1 and self.clip_ratio > 0, (
    ...
    def forward(self, x, R1=None, R2=None, transpose=False):
        ...
        if self.online_full_had:
            if self.fp32_had:  # Full Hadamard in FP32
                x = had_mul(x.float(), self.had_K, self.K).to(x_dtype)
            else:  # Full Hadamard in FP16
                x = had_mul(x, self.had_K, self.K)
...
class WeightQuantizer(torch.nn.Module):
    ...
    # TODO: This should be better refactored into `forward`, which applies quantize and
    # dequantize. A new method `quantize` should be added (if needed) to return the
    # quantized integers and scales, like in ActQuantizer.
    def quantize(self, x):
        x_dtype = x.dtype
...
def find_qlayers(
    module,
    layers=[torch.nn.Linear, ActQuantWrapper, QuantizeLinearNEW],
    name: str = "",
):
