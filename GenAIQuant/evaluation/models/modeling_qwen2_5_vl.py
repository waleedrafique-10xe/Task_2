...
class Qwen2_5_VLVisionFlashAttention2(nn.Module):
    ...
    def forward(
        ...
        # ==================== MOD BLOCK ==================
        k = self.rot_embedding_k(k)
        q = self.rot_embedding_q(q)
        # ==================== MOD BLOCK ==================
...
# KAKAROT: Need to apply QDQ within this function -- turn this into a Module
# Will probably need to remove the qdq functions created aftert this function
# is called as well
#
...
class Qwen2_5_VLVisionSdpaAttention(nn.Module):
    ...
    def forward(
        ...
        # NO_QDQ -- Can't add QDQ hooks to pytorch operators
...
class Qwen2_5_VLVisionBlock(nn.Module):
    def __init__(self, config, attn_implementation: str = "sdpa") -> None:
        ...
        # ==================== MOD BLOCK ==================
        self.attn_residual = PassThrough()
        self.mlp_residual = PassThrough()
        # ==================== MOD BLOCK ==================
    ...
    def forward(
        ...
        # ==================== MOD BLOCK ==================
        hidden_states = self.attn_residual(hidden_states)
        # ==================== MOD BLOCK ==================
...
class Qwen2_5_VLAttention(nn.Module):
    ...
    def forward(
        ...
        # ==================== MOD BLOCK ==================
        # bias correction for qwen2_5_vl_7b
        if self.num_heads == 28 and (self.layer_idx == 0 or self.layer_idx == 27):
            indices = torch.arange(self.k_proj.bias.size(0)).reshape(1, 4, 1, 128)
            mask = indices % 64 >= 50
            k_proj_bias_masked = self.k_proj.bias.clone().reshape(1, 4, 1, 128)
            k_proj_bias_masked[~mask] = 0
            key_states = key_states - k_proj_bias_masked
        # ==================== MOD BLOCK ==================

        # KAKAROT: shouldn't position embedidngs be quantized
        cos, sin = position_embeddings

        # KAKAROT: Add hook here?
        ...
        if past_key_value is not None:
            ...
            # KAKAROT: is there a need to add another QDQ hook here?
            # If we're getting it should already be quantized?

        # repeat k/v heads if n_kv_heads < n_heads

        # KAKAROT: Probably need to do QDQ here as well
        ...
        if attention_mask is not None:  # no matter the length, we just slice it
            ...
            # KAKAROT: Should probably add QDQ here as well
...
class Qwen2_5_VLFlashAttention2(Qwen2_5_VLAttention):
    ...
    def forward(
        ...
        # repeat k/v heads if n_kv_heads < n_heads
        # NO_QDQ: No data distribution changes here
        key_states = repeat_kv(key_states, self.num_key_value_groups)
        value_states = repeat_kv(value_states, self.num_key_value_groups)
        ...
        # KAKAROT: This function is not defined within the modeling file
        # So not going in side this to add modules
...
class Qwen2_5_VLSdpaAttention(Qwen2_5_VLAttention):
    ...
    def forward(
        ...
        # NO_QDQ: Transpose does not change data distribution
        query_states = query_states.view(bsz, q_len, -1, self.head_dim).transpose(1, 2)
        key_states = key_states.view(bsz, q_len, -1, self.head_dim).transpose(1, 2)
        value_states = value_states.view(bsz, q_len, -1, self.head_dim).transpose(1, 2)
        ...
        # NO_QDQ -- repeat_kv dies not change distributions
        key_states = repeat_kv(key_states, self.num_key_value_groups)
        value_states = repeat_kv(value_states, self.num_key_value_groups)
        ...
        # in SDPA to support both torch.compile's dynamic shapes and full graph
        ...
        # KAKAROT: sdpa is implemented within pytorch.
        # Not changing the implementation details within this block
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        hidden_states = outputs[0]  # Should add QDQ here?
